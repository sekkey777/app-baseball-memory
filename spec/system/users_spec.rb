...
RSpec.describe 'Users', type: :system do
  ...
  describe 'ユーザー詳細、編集ページ' do
    ...
    context 'ゲストユーザーの場合' do
      ...
      it 'ゲストユーザーの場合は、編集ボタンが表示されないこと' do
        expect(page).not_to have_content('戻る')
        expect(page).not_to have_content('更新')
      end

      it 'ゲストユーザーの場合は、編集画面に遷移できないこと' do
        guest_user = User.find_by(name: 'guest_user')
        visit edit_user_path(guest_user)
        expect(page).to have_current_path(user_path(guest_user))
      end
    end
